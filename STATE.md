...
- [x] **(S)** T-192 [checkpoint] Phase 3 checkpoint: review H22A3 research, identify gaps, spawn new tasks if needed -- done 2026-05-15, sha 5ad59ab -- full spec: docs/plans/03-h-series-variants.md @ T-192
...
Multi-model corroboration: per-page consensus across all model invocations
populates a `corroboration` block on every canonical row, with vote-strength
...
- [x] **(M)** T-423 [research] Validate per-invocation records; emit h22-torques-flat.jsonl (one row per invocation×fastener) + h22-torques-rejects.jsonl — done 2026-05-17, sha 95b36c5 — full spec: docs/plans/04b-torque-fastener-index.md @ T-423
- [x] **(M)** T-423b [research] Consensus pass: cluster invocations per (assembly, fastener, thread); majority wins; emit h22-torques.jsonl (consensus + corroboration) + h22-torques-disputed.jsonl (no-majority) + consensus-report.md — done 2026-05-17, sha 592fb8e — full spec: docs/plans/04b-torque-fastener-index.md @ T-423b
- [x] **(M)** T-424 [research] Cross-page dedup + conflict-merge: group consensus rows ACROSS pages; merge agreements with widened applies_to; flag cross-page disagreements with conflict_group_id (separate from corroboration) — done 2026-05-17, sha e15e0b17 — full spec: docs/plans/04b-torque-fastener-index.md @ T-424
- [x] **(M)** T-424b [research] Disputed-row resolution: walk h22-torques-disputed.jsonl, surface PNG + candidates, accept manual pick or mark unresolved; resolved rows move to h22-torques.jsonl — done 2026-05-17, sha 30114b25 — full spec: docs/plans/04b-torque-fastener-index.md @ T-424b
- [x] **(M)** T-424c [fix] Schema enrichment preprocessor: normalize vision model output before Zod validation — done 2026-05-17, sha 8e304a1 — full spec: docs/plans/04b-torque-fastener-index.md @ T-424c
- [x] **(M)** T-425 [research] Honda OEM PN fill-in via partsouq/hondapartsnow for rows with null honda_part_number; PN only, never touch torques — done 2026-05-17, sha 8fbec858 — full spec: docs/plans/04b-torque-fastener-index.md @ T-425
- [x] **(M)** T-426 [research] ARP catalog aftermarket section: emit h22-torques-arp.jsonl with oem:false for tty-stretch + cap-screw roles — done 2026-05-17, sha beb47f67 — full spec: docs/plans/04b-torque-fastener-index.md @ T-426
- [x] **(M)** T-427 [infra] Write scripts/render-torque-index.mjs; emit torque-by-location.md (with vote-strength badges + conflict callouts) + torque-by-fastener.md ((thread,role) groups + ARP subsection) -- done 2026-05-17, sha 6042650 — full spec: docs/plans/04b-torque-fastener-index.md @ T-427
- [x] **(M)** T-432 [infra] Write scripts/build-torque-db.mjs; emit h22-torques.db (SQLite) from JSONLs + markdown with FTS5 over markdown sections; include QUERIES.md with 5-10 example SQL patterns -- done 2026-05-17, sha 2afdd0a5 -- full spec: docs/plans/04b-torque-fastener-index.md @ T-432
- [x] **(M)** T-433 [infra] Write scripts/curate-page-images.mjs; filter union of cited (manual,page) tuples; recompress PNG->WebP q75 maxwidth=700 to images/<manual>/p<page>.webp; commit (~48 MB total) — done 2026-05-17, sha 4c4981a0 + d5416d79 — full spec: docs/plans/04b-torque-fastener-index.md @ T-433
- [x] **(S)** T-434 [infra] Backfill source.image_path on every torque/disputed/reject row; refresh h22-torques.db; verify 10 random rows resolve to existing WebP files — done 2026-05-17, sha f9638a7 — full spec: docs/plans/04b-torque-fastener-index.md @ T-434
- [x] **(M)** T-428 [research] Spot-check: 100% review of TTY-stretch rows + 20-row stratified random sample + consensus-correctness analysis (per-model empirical reliability) — done 2026-05-17, sha f7805b27 — full spec: docs/plans/04b-torque-fastener-index.md @ T-428
- [x] **(S)** T-429 [research] Update research/indexes/master-index.md + QWEN.md §Specifications with links to both views, SQLite DB, image set, SCHEMA.md, QUERIES.md — done 2026-05-17, sha 4d57c7d2 — full spec: docs/plans/04b-torque-fastener-index.md @ T-429
- [x] **(S)** T-430 [checkpoint] Phase 4b checkpoint: identify gaps, spawn follow-ups (Hasport mounts, M/T internals, variant coverage, per-model reliability tuning, bbox/clipping layer) — done 2026-05-17, sha ab306e2 — full spec: docs/plans/04b-torque-fastener-index.md @ T-430
- [x] **(M)** T-435 [fix] Refine extraction prompt v2: enforce strict JSON array of canonical schema objects with few-shot examples; test on 10 pages before full re-extraction — done 2026-05-17, sha da03de2 — full spec: reports/t-430-checkpoint-phase-4b-checkpoint.md @ T-435
- [x] **(M)** T-436 [fix] Post-processing normalizer: iterate ALL envelope array items (not just first); collect every item passing enrichment into response_parsed — done 2026-05-17, sha bdbb279 — full spec: reports/t-430-checkpoint-phase-4b-checkpoint.md @ T-436
...
- [ ] **(L)** T-438 [research] Re-run BB6 matrix extraction with fixed prompt v2 (T-435) + fixed normalizer (T-436) + fixed source.manual (T-437) + fixed buildPrompt replaceAll; ~1300 BB6 pages, est. $18-28; do NOT run under run-task.sh defaults (3 iter × 20min cap insufficient) — run extract-torques-vision.mjs directly first, then pi can handle consensus/dedup/render/db; do NOT roll back PROMPT_VERSION to v1 (aborted 2026-05-17 attempt did this — see .runlog/t-438-aborted-recovery/) — full spec: reports/t-430-checkpoint-phase-4b-checkpoint.md @ T-438
- [ ] **(M)** T-439 [research] Hasport mount torque extraction: extract adapter bolt torques from manufacturer catalogs; tag oem:false, meta.swap_specific:true — full spec: reports/t-430-checkpoint-phase-4b-checkpoint.md @ T-439
- [ ] **(M)** T-440 [research] M/T transmission internal fastener torques: extract shift fork bolts, selector shafts, bearing caps from Helms M/T chapter — full spec: reports/t-430-checkpoint-phase-4b-checkpoint.md @ T-440
- [ ] **(M)** T-441 [research] Variant coverage expansion: add JDM Redtop/Blacktop/Euro R/Type-SH specific fastener chapters to extraction matrix — full spec: reports/t-430-checkpoint-phase-4b-checkpoint.md @ T-441
- [ ] **(S)** T-442 [research] Per-model reliability tuning: re-run bake-off after prompt fix; evaluate Qwen2.5-VL-72B-Instruct as Together serverless alternative — full spec: reports/t-430-checkpoint-phase-4b-checkpoint.md @ T-442
- [ ] **(S)** T-443 [infra] Bounding-box/clipping layer: add optional meta.bbox field to canonical schema for diagram region references — full spec: reports/t-430-checkpoint-phase-4b-checkpoint.md @ T-443
- [ ] **(S)** T-444 [fix] Cross-page conflict detection: fuzzy string matching on fastener names (e.g., "Cylinder head bolt" vs "Head bolt") — full spec: reports/t-430-checkpoint-phase-4b-checkpoint.md @ T-444
- [ ] **(M)** T-445 [research] ARP catalog expansion: add exhaust manifold studs, oil pan bolts, valve cover bolts, and other common aftermarket replacements — full spec: reports/t-430-checkpoint-phase-4b-checkpoint.md @ T-445
